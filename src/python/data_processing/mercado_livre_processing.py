...
def _create_searchable_string(row):
    ...
    # Safely evaluate and join 'destaques'
    if 'destaques' in row and row['destaques']:
        try:
            destaques_list = ast.literal_eval(row['destaques'])
            if isinstance(destaques_list, list):
                search_string += ' '.join(destaques_list) + ' '
        except (ValueError, SyntaxError):
            search_string += str(row['destaques']) + ' '

    # Safely evaluate and join 'caracteristicas_completas'
    if 'caracteristicas_completas' in row and row['caracteristicas_completas']:
        try:
            caracteristicas_dict = ast.literal_eval(row['caracteristicas_completas'])
            if isinstance(caracteristicas_dict, dict):
                search_string += ' '.join(f"{k} {v}" for k, v in caracteristicas_dict.items())
        except (ValueError, SyntaxError):
            search_string += str(row['caracteristicas_completas']) + ' '
...
def _extract_processor(text):
    ...
    # Expanded to include Celeron, Pentium, and more variations
...
def process_mercado_livre_data(df):
    print("Processing Mercado Livre data with unified extraction...")
